        ...
        <div className="space-y-6 pr-6">
          {education.map((edu) => (
            <Card key={edu.id} className="bg-transparent border-stone-400/50">
              <CardHeader>
                <CardTitle className="text-xl font-semibold text-page-foreground flex items-center gap-3">
                    <School className="h-6 w-6 text-primary"/>
                    {edu.institution}
                </CardTitle>
                <CardDescription className="text-page-foreground/70 flex flex-col sm:flex-row sm:items-center gap-x-4 gap-y-1 text-sm pt-1">
                    <span className="flex items-center"><Calendar className="mr-2 h-4 w-4"/>{edu.session}</span>
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="flex items-center gap-2 text-page-foreground/80">
