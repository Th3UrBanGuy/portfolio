    ...
    <div className="h-full flex flex-col">
      ...
      <ScrollArea className="flex-grow -mr-6">
        <div className="space-y-6 pr-6">
          {privateInfo.sections.map(section => (
            <Card key={section.id} className='bg-transparent border-stone-400/50'>
              <CardHeader>
                <CardTitle className="flex items-center gap-3 font-headline text-lg text-page-foreground">
                  <DynamicIcon name={section.icon_name || 'Users'} className="h-5 w-5 text-primary"/>
                  {section.title}
                </CardTitle>
              </CardHeader>
                  ...
                  {section.fields.map(field => (
                    !field.isSecret && (
                      <div key={field.id} className="flex items-start gap-3">
                        ...
                        <div>
                          <p className='text-page-foreground/70'>{field.label}</p>
                          <p className="font-semibold text-page-foreground/90">{field.value}</p>
                        </div>
                      </div>
                    )
                  ))}
